...
class WoePlanetExporter(object):
    ...
    def export_as_geojson(self, path):
        rsp = helpers.scan(self.es, index=self.index, scroll='5m', size=self.scroll_size)
        for num,doc in enumerate(rsp):
            ...
            if src['woe:placetype'] == 0:
                if 'woe:superseded_by' in src:
                    ...
                    if doc:
                        ...
                        logging.error('%d: Cannot find superseded_by link to %d!' % (src['woe:id'], src['woe:superseded_by']))
                else:
                    logging.error('%d: %s placetype with no superseded_by linkage!' % (src['woe:id'], src['woe:placetype']))
            ...
            if os.path.isfile(outpath) and not self.args.force:
                with open(outpath, 'r') as ifh:
                    data = json.load(ifh)
    ...
    def get_repo(self, root, doc):
        pt = doc['woe:placetype']
        ptn = doc['woe:placetype_name'].lower()
        if 'iso:country' in doc:
            iso = doc['iso:country'].lower()
        else:
            iso = 'zz'

        if pt == 11 and (iso == 'gb' or iso == 'ca' or iso == 'pt' or iso == 'jp'):    # zip
            reponame = 'woeplanet-%s-%s-%s' % (ptn, iso, doc['woe:name'][0:1].lower())
        else:
            reponame = 'woeplanet-%s-%s' % (ptn, iso)
...
def main():
    parser = argparse.ArgumentParser(prog='export-woeplanet-geojson', description='Exports a WoePlanet ES index as individual GeoJSON files')
    parser.add_argument("-e", "--elasticsearch", dest="es", help="your ES endpoint; default is localhost:9200", default='localhost:9200')
    parser.add_argument("-f", "--force", dest="force", action="store_true", help="force export a WOEID, even if no changes have been made since the last export; default is false", default=False)
